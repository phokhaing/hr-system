          ...
          <div class="box-body">
            ...
            <div class="form-group row">
              <div class="col-sm-6 col-md-6 col-lg-6">
                <button
                  ...
                  v-if="can('un_post_payroll_half_month')"
                >
                  <i class="fa fa-filter"></i> Filter Un Post
                  <!-- {{ getCountFilters() }} -->
                </button>
                <button
                  :disabled="!this.getHalfPayrollPosted.length > 0"
                  class="btn btn-primary margin-r-5"
                  id="btnPostPayroll"
                  @click="unPostPayroll()"
                  v-if="can('un_post_payroll_half_month')"
            ...
            <div>
              ...
              <list-checking-payroll
                :temp-payrolls="this.getHalfPayrollPosted"
                @toggleBlockUnBlock="blockOrUnBlock"
              >
              </list-checking-payroll>
              <h5
                v-bind:style="{ textAlign: 'center' }"
                v-if="!this.getHalfPayrollPosted.length > 0"
              >
                Empty
              </h5>
            </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters({
      getHalfPayrollPosted: "payroll/getHalfPayrollPosted",
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    ...mapActions({
      ...
      UN_POST_PAYRLL_HALF_MONTH: "payroll/UN_POST_PAYRLL_HALF_MONTH",
      GET_POSTED_PAYRLL_HALF_MONTH: "payroll/GET_POSTED_PAYRLL_HALF_MONTH",
    }),
    ...
    async createTempFullPayroll() {
      await this.GET_POSTED_PAYRLL_HALF_MONTH({
    ...
    async unPostPayroll() {
      await this.UN_POST_PAYRLL_HALF_MONTH({
        company_code: this.companyCode,
        branch_department_code: this.branchDepartmentCode,
        staff_personal_info_id: this.staffPersonalInfoId,
      });
