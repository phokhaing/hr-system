...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-heading"><label for="">Edit branch</label></div>
        <div class="panel-body">
            {{ session()->get('message') }}
            <div class="row">
                <div class="col-sm-12">
                    <form action="{{ url('/branch/'.$branch->id.'/update') }}" method="POST" role="form">
                        {{ Form::token() }}
                            ...
                            <div class="form-group col-sm-6 col-md-6 @if($errors->has('company_id')) has-error @endif">
                                ...
                                <select class="form-control" name="company_id" id="company_id" >
                                    ...
                                    @foreach($companies as $key => $value)
                                        <option @if($branch->company_id == $value->id) selected @endif value="{{ $value->id }}">{{ $value->code.'-'.$value->name_kh }}</option>
                                    @endforeach
                                </select>
                            </div>
