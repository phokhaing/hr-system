...
class ByStaffPerCompanySheet implements FromView, ShouldAutoSize, WithTitle, WithDrawings, WithColumnFormatting
{
    ...
    public function view(): View
    {
        if ($this->isTempPayroll) {
            $payroll_object = TempPayroll::exportTempPayrollByCompany($this->company_code, date('Y-m-d'));
        } else {
            $payroll_object = Payroll::exportPayrollByCompany($this->company_code, $this->transaction_date);
        }
        $transactionDate = @$payroll_object[0]->transaction_date;

        return view('reports.payroll.full_month.get_by_staff_excel', [
